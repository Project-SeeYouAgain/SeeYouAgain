...
function Heart({ isActive, className, productId }: ButtonProps) {
    ...
    return (
        <>
            {isCart ? (
                <AiFillHeart className={` w-6 h-6 ${className}`} color={'#5669FF'} onClick={(event: React.MouseEvent) => ClickHeart(event)} />
            ) : (
                <AiOutlineHeart className={`w-6 h-6 ${className}`} color={'#5669FF'} onClick={(event: React.MouseEvent) => ClickHeart(event)} />
            )}
        </>
    );
}
